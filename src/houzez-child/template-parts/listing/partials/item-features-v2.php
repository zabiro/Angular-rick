<ul class="item-amenities <?php echo houzez_v2_meta_type(); ?>">
	...
	if ($data_composer) {
        ...
        $data_composer = array_merge(array_flip(array('area-size','bed','bath', 'garage')), $data_composer);
		...
		foreach ($data_composer as $key=>$value) { $i ++;
			...
			if(in_array($key, houzez_listing_composer_fields())) {
                ...
                $output .= get_template_part('template-parts/centrosur/proyectos/tipo-construccion-item');
				echo $output;
			}
        ...
        get_template_part('template-parts/centrosur/proyectos/tipo-construccion-item');
	}
	?>
</ul>
