...
if( !function_exists('houzez_show_stripe_form_per_listing') ) {
    function houzez_show_stripe_form_per_listing( $postID, $price_submission, $price_featured_submission ) {
        ...
        print '
    <form action="' . esc_url($processor_link) . '" method="post" id="stripe_form_simple_listing">
        <div class="houzez_stripe_simple">
            <script src="https://checkout.stripe.com/checkout.js"
            class="stripe-button"
            data-key="' . $stripe_publishable_key . '"
        ...
        <input type="hidden" id="propID" name="propID" value="' . intval($postID) . '">
        <input type="hidden" id="submission_pay" name="submission_pay" value="1">
        <input type="hidden" name="userID" value="' . intval($userID) . '">
        <input type="hidden" name="relist_mode" value="' . esc_attr($relist_mode) . '">
        <input type="hidden" id="pay_ammout" name="pay_ammout" value="' . $price_submission . '">
    </form>

    <form action="' . esc_url($processor_link) . '" method="post" id="stripe_form_featured_listing">
        <div class="houzez_stripe_simple">
            <script src="https://checkout.stripe.com/checkout.js"
            class="stripe-button"
            data-key="' . $stripe_publishable_key . '"
...
if( !function_exists('houzez_stripe_cancel_subscription') ) {
    function houzez_stripe_cancel_subscription() {
        ...
        update_user_meta($current_user->ID, 'fave_stripe_user_profile', '');
    }
}
...
if( !function_exists('houzez_cancel_stripe') ) {
    function houzez_cancel_stripe() {
        ...
        update_user_meta($userID, 'fave_stripe_user_profile', '');
